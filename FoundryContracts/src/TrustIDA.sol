...
contract TrustIDA is SuperAppBase, Ownable {
    ...
    function makeOnetimebatch(
        /** could also make onetime payment*/
    ...
    //called when an employer pays a onetime index
    function addOneTimeInvoiceToIndex(uint256 invoiceId_) external onlyOwner {
        ITrust.Invoice memory invoice = trusContract.viewInvoice(invoiceId_);
        DistributionInfo storage distributionInfor = timeStampDistributable[
            invoice.deadline
        ];
        ISuperToken _token = ISuperToken(invoice.token);
        if (distributionInfor.isDistributable == false) {
            _host.callAgreement(
                _ida,
                abi.encodeWithSelector(
            ...
            _host.callAgreement(
                ...
                abi.encodeWithSelector(
                    _ida.updateSubscription.selector,
                    _token,
                    uint32(invoice.deadline),
                    recipient,
                    uint128(invoice.amount),
                    new bytes(0) // placeholder ctx
                ),
                new bytes(0) // user data
            );
            amountTodistribute[timeDue_] += invoice.amount;
            address[] storage _addresses = distributionInfor._receivers;
            _addresses.push(recipient);
            timeStampDistributable[_time] = DistributionInfo(true, _addresses);
        } else {
            ...
            amountTodistribute[invoice.deadline] += invoice.amount;
            address[] storage _addresses = distributionInfor._receivers;
            _addresses.push(invoice.receiver);
            timeStampDistributable[invoice.deadline]._receivers = _addresses;
        }
    }
